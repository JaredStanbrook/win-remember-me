...
def _edge_meta(window: Dict) -> Dict:
...
def _edge_cdp_hint(window: Dict) -> Optional[int]:
    edge = window.get("edge") if isinstance(window.get("edge"), dict) else {}
    try:
        hint = int(edge.get("cdp_window_hint") or 0)
    except (TypeError, ValueError):
        return None
    return hint if hint > 0 else None


def _set_edge_cdp_hint(window: Dict, cdp_window_id: Optional[int]) -> None:
    if not isinstance(cdp_window_id, int) or cdp_window_id <= 0:
        return
    _edge_meta(window)["cdp_window_hint"] = cdp_window_id
...
def _assign_edge_tabs_to_windows(windows: List[Dict], tabs: List[Dict], target_windows: Optional[List[Dict]] = None) -> None:
    ...
    for idx, window in enumerate(edge_windows):
        ...
        hint = _edge_cdp_hint(window)
        if hint is not None:
            windows_by_hint[hint] = idx
    ...
    for cdp_window_id in sorted(tabs_by_window_id.keys()):
        grouped_tabs = tabs_by_window_id[cdp_window_id]
        match_idx: Optional[int] = windows_by_hint.get(cdp_window_id)

        if match_idx is None:
            for tab in grouped_tabs:
                tab_title = _normalize_edge_window_title(str(tab.get("title") or ""))
                for idx in windows_by_title.get(tab_title, []):
        ...
        _set_edge_cdp_hint(edge_windows[match_idx], cdp_window_id)
        for tab in grouped_tabs:
...
def _assign_tabs_to_edge_windows_stably(edge_windows: List[Dict], tabs: List[Dict]) -> None:
    ...
    for tab in tabs:
        ...
        tab_cdp_id = tab.get("window_id") if isinstance(tab.get("window_id"), int) else None
        if tab_cdp_id is not None:
            assigned_window = windows_by_hint.get(tab_cdp_id)
        ...
        if tab_cdp_id is not None:
            _set_edge_cdp_hint(assigned_window, tab_cdp_id)
    ...
    if capture_edge_tabs:
        ...
        for window in windows:
            ...
            if window.get("edge_tabs"):
                _edge_meta(window)["session_port"] = int(edge_debug_port)
...
def _score_components(candidate: Dict, target: Dict) -> Dict[str, int]:
    exe_score = 0
    process_score = 0
    class_score = 0
    title_score = 0
    ...
    t_exe = str(target.get("exe") or "").strip().lower()
    c_exe = str(candidate.get("exe") or "").strip().lower()
    if t_exe and c_exe and t_exe == c_exe:
        exe_score = 50

    t_proc = str(target.get("process_name") or "").strip().lower()
    c_proc = str(candidate.get("process_name") or "").strip().lower()
    if t_proc and c_proc and t_proc == c_proc:
        process_score = 25

    t_class = str(target.get("class_name") or "").strip().lower()
    c_class = str(candidate.get("class_name") or "").strip().lower()
    if t_class and c_class and t_class == c_class:
        class_score = 15

    t_title = str(target.get("title") or "").strip().lower()
    c_title = str(candidate.get("title") or "").strip().lower()
    edge_title_deweighted = False
    if t_title and c_title:
        if t_proc == "msedge.exe":
            edge_title_deweighted = True
        else:
            if t_title == c_title:
                title_score = 30
            elif t_title in c_title or c_title in t_title:
    ...
    target_rect = tuple(target.get("rect") or target.get("normal_rect") or ())
    candidate_rect = tuple(candidate.get("rect") or candidate.get("normal_rect") or ())
    if len(target_rect) == 4 and len(candidate_rect) == 4:
        if _is_close_rect(candidate_rect, target_rect, 40):
            geometry_score = 20
        elif _is_close_rect(candidate_rect, target_rect, 120):
            geometry_score = 10

    total = exe_score + process_score + class_score + title_score + geometry_score
    return {
        "exe": exe_score,
        ...
        "total": total,
        "edge_title_deweighted": 1 if edge_title_deweighted else 0,
    }


def _match_candidates_with_scores(target: Dict, current: List[Dict], used_hwnds: set) -> List[Dict]:
    ranked: List[Dict] = []
    for candidate in current:
        if candidate.get("hwnd") in used_hwnds:
            continue
        components = _score_components(candidate, target)
        ranked.append({
            "candidate": candidate,
            "components": components,
            "score": int(components.get("total") or 0),
        })
    ranked.sort(key=lambda item: item["score"], reverse=True)
    return ranked


def _print_match_diagnostics(target: Dict, ranked: List[Dict], top_n: int = 3) -> None:
    t_proc = str(target.get("process_name") or "")
    t_title = str(target.get("title") or "")
    print(f"[DIAG] Target: proc={t_proc} title={t_title}")
    if not ranked:
        print("[DIAG]   No candidates")
        return

    for idx, item in enumerate(ranked[:max(1, int(top_n))], start=1):
        candidate = item["candidate"]
        components = item["components"]
        edge_note = " edge_title_deweighted=true" if components.get("edge_title_deweighted") else ""
        print(
            "[DIAG]   "
            f"#{idx} hwnd={candidate.get('hwnd')} score={item['score']} "
            f"(exe={components.get('exe', 0)}, process={components.get('process', 0)}, "
            f"class={components.get('class', 0)}, title={components.get('title', 0)}, "
            f"geometry={components.get('geometry', 0)})"
            f"{edge_note}"
        )
...
def _best_match(target: Dict, current: List[Dict], used_hwnds: set, min_score: int) -> Tuple[Optional[Dict], int]:
    ranked = _match_candidates_with_scores(target, current, used_hwnds)
    if not ranked:
        return None, -1
...
def _find_existing_edge_hwnd_for_target(target: Dict, current: List[Dict], used_hwnds: set, min_score: int) -> Optional[int]:
    edge_candidates = [
        candidate for candidate in current
        if str(candidate.get("process_name") or "").lower() == "msedge.exe"
    ]
    ranked = _match_candidates_with_scores(target, edge_candidates, used_hwnds)
    if not ranked:
        return None
    best = ranked[0]
    if int(best.get("score") or 0) < int(min_score):
        return None
    hwnd = best["candidate"].get("hwnd")
    if hwnd is None:
        return None
    used_hwnds.add(hwnd)
    return int(hwnd)
...
def restore_layout(path: str, mode: str = "basic", diagnostics: bool = False, diagnostics_top_n: int = 3) -> None:
    with open(path, "r", encoding="utf-8") as f:
        data = json.load(f)
    ...
    for t in targets:
        ranked = _match_candidates_with_scores(t, current, used_hwnds)
        if diagnostics:
            _print_match_diagnostics(t, ranked, top_n=diagnostics_top_n)
        if not ranked:
            missing.append(t)
            continue

        best = ranked[0]["candidate"]
        _best_score = int(ranked[0]["score"])
        if _best_score < min_score:
            missing.append(t)
            continue
...
def main():
    ...
    p_restore.add_argument("--diagnostics", action="store_true", help="Print per-target match diagnostics")
    p_restore.add_argument("--diagnostics-top", type=int, default=3, help="How many match candidates to print (default: 3)")
    ...
    if args.cmd == "save":
        ...
        restore_layout(args.json_path, mode=args.mode, diagnostics=args.diagnostics, diagnostics_top_n=args.diagnostics_top)
    elif args.cmd == "help":
        if args.full:
            parser.print_help()
            return
